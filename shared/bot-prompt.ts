...
/**
 * Hermes harness loop — short, always-on structure for multi-tool turns.
 *
 * Not a second personality and not a fork of Hermes Agent. It sits beside provenance and computer
 * guidance so every built-in Bot plans briefly, stays on the allowlist, and stops looping when
 * stuck or when the person interrupts with a new message.
 */
const HERMES_GUIDANCE_LINES = [
  "Work in a short loop: plan, act with one tool, observe the result, then either the next tool",
  "or your answer. Prefer a visible plan for anything that needs more than two tool calls.",
  "Call hermes_plan once at the start of that kind of turn with a goal and a few concrete steps;",
  "update step status as you go rather than rewriting the whole plan.",
  "",
  "Stay on tools you were offered. Do not invent tool names. If a tool is refused or denied, say",
  "so and stop that route — do not retry the same call in a circle.",
  "",
  "Shell commands need the person's approval when Ask is on. Propose the exact command, wait for",
  "Allow or Deny, and never treat a declined command as success.",
  "",
  "A new message from the person interrupts this turn. Drop unfinished work, do not finish the",
  "old plan in the background, and answer the new message.",
  "",
  "When the conversation is long or you are near a context limit, compress: keep the goal, open",
  "decisions, and file paths; drop repeated tool dumps. Say in one line that you compacted.",
  "Answer briefly when the work is done.",
];

export const HERMES_GUIDANCE = HERMES_GUIDANCE_LINES.reduce<string[]>(
  (paragraphs, line) => {
    if (line === "") {
      paragraphs.push("");
      return paragraphs;
