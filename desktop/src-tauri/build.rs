fn main() {
    ...
    // Set by `bun run connect` / `bun run connect:package`. The packaged exe
    // keeps the flag even when the person double-clicks it with a clean environment.
    println!("cargo:rerun-if-env-changed=CONNECT_PRODUCT_WINDOW");
    println!("cargo:rustc-check-cfg=cfg(connect_product)");
    if std::env::var("CONNECT_PRODUCT_WINDOW").ok().as_deref() == Some("1") {
        println!("cargo:rustc-cfg=connect_product");
    }
    tauri_build::build()
}
