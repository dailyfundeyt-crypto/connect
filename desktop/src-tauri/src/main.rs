...
struct Shell {
    ...
    /// The product window (Connect UI, native frame). Closing it quits the shell and
    /// leaves a service this shell did not install running.
...
#[tauri::command]
fn attach_connect_window<R: tauri::Runtime>(app: tauri::AppHandle<R>) -> Result<(), String> {
    connect_product::attach(app)
}
    ...
    tauri::Builder::default()
        ...
        .on_page_load(|webview, _| {
            ...
            if webview
                ...
                .load(std::sync::atomic::Ordering::SeqCst)
            {
                let _ = webview.window().set_title("Connect");
            }
        })
        ...
        .on_window_event(|window, event| {
            if let tauri::WindowEvent::CloseRequested { api, .. } = event {
                // A program ends when its window closes. The setup shell still hides to
                // the tray, because that window is a supervisor with a stack underneath.
                if window
                    .state::<Shell>()
                    .connect_product
                    .load(std::sync::atomic::Ordering::SeqCst)
        ...
        .setup(|app| {
            ...
            let product = connect_product::enabled();
        ...
        .run(|app, event| {
            match event {
                ...
                tauri::RunEvent::ExitRequested { api, code, .. } => {
                    // The product window did not install the deployment. Quitting it must
                    // not compose-down a stack the person started themselves.
                    if app
                        .state::<Shell>()
                        .connect_product
                        .load(std::sync::atomic::Ordering::SeqCst)
