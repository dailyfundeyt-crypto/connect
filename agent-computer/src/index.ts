...
/** How often a cold headed browser is asked again before the first paint is ready. */
const SCREENSHOT_ATTEMPTS = 3;
const SCREENSHOT_RETRY_MS = 250;

/**
 * Take a PNG of the Bot's page, retrying briefly when headed Chromium is not paint-ready yet.
 *
 * A fresh Ubuntu computer under Xvfb can answer `/health` and even navigate before CDP can capture
 * a frame. Without this, the first Watch poll after `/ensure` fails and the panel flashes an error
 * that the next poll clears. The retries stay short so a real failure still surfaces quickly.
 */
async function capturePageScreenshot(
  botId: string,
): Promise<{ target: Page; buffer: Buffer }> {
  ...
  for (let attempt = 1; attempt <= SCREENSHOT_ATTEMPTS; attempt += 1) {
    ...
    try {
      ...
      lastError = error;
      if (attempt === SCREENSHOT_ATTEMPTS) break;
      await new Promise((resolve) => setTimeout(resolve, SCREENSHOT_RETRY_MS));
    }
  }
...
serve<StreamData>({
  ...
  async fetch(request, server) {
    ...
    if (url.pathname === "/screenshot" && request.method === "GET") {
      try {
        /*
         * Headed Chromium on a fresh Xvfb can accept navigation before CDP can paint a frame.
         * One or two short retries absorb that race so "Watch this Bot's screen" does not flash
         * a 502 on the first poll of a newly ensured Ubuntu computer.
         */
        const { target, buffer } = await capturePageScreenshot(botId);
