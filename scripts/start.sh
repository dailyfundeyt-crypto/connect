...
# Soft variant: warn and continue (used when API keys are absent).
wait_for_optional() {
  local url="$1" name="$2" tries="${3:-15}"
  for _ in $(seq 1 "$tries"); do
    curl -fsS --max-time 3 "$url" >/dev/null 2>&1 && { green "  $name ready"; return 0; }
    sleep 1
  done
  info "  $name not ready at $url — continuing without it"
  return 0
}
...
# Docker migrate sometimes hangs on the spinner under non-TTY redirects.
# Prefer it, then fall back to a local Bun migrate against localhost Postgres.
if ! timeout 90 docker compose run --rm --build migrate >"$LOGS/migrate.log" 2>&1; then
  info "  Docker migrate did not finish — trying local bun migrate…"
  if ! (cd "$ROOT/server" && bun --env-file=../.env drizzle-kit migrate --config=drizzle.config.ts) \
      >"$LOGS/migrate-local.log" 2>&1; then
    red "  Migrations did not apply. The database is not the schema this server expects."
    red "  Log: $LOGS/migrate.log (and $LOGS/migrate-local.log)"
    exit 1
  fi
  green "  migrations applied via local bun"
fi
wait_for "http://localhost:$COMPUTER_PORT/health" "agent-computer"
# Without model / Intelligence keys the Bot images exit immediately. Allow a
# degraded laptop boot so the UI/API can still come up for setup.
if [ "${OPENBOT_FORCE_START:-}" = "1" ]; then
  if [ "$BOT_PROVIDER" != "anthropic" ]; then
    wait_for_optional "http://localhost:$BOT_PORT/health" "agent-bot"
  fi
  wait_for_optional "http://localhost:$LANGGRAPH_PORT/health" "agent-langgraph"
else
  if [ "$BOT_PROVIDER" != "anthropic" ]; then
...
if [ -z "$INFO" ]; then
  if [ "${OPENBOT_FORCE_START:-}" = "1" ]; then
    info "  /api/copilotkit/info unreachable — continuing (OPENBOT_FORCE_START=1)"
  else
    red "  /api/copilotkit/info unreachable"
    exit 1
  fi
else
python3 - "$INFO" "${OPENBOT_FORCE_START:-}" <<'PY'
import json, sys
...
if status != "valid":
    ...
    if not force:
        raise SystemExit(1)
    print("\033[2m  OPENBOT_FORCE_START=1 — continuing without a valid licence\033[0m")
elif not agents:
    print("\033[31m  No Bots registered.\033[0m")
    if not force:
        raise SystemExit(1)
    print("\033[2m  OPENBOT_FORCE_START=1 — continuing without registered Bots\033[0m")
else:
