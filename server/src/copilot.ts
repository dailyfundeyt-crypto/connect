...
export function builtInAgentConfiguration(
  ...
  return {
    ...
    prompt: [
      ...
      PROVENANCE_GUIDANCE,
      /*
       * Hermes harness: plan → tool → observe → reply. Unconditional for built-in Bots the way
       * provenance is — it is how to work a multi-tool turn, not a capability a deployment may omit.
       */
      HERMES_GUIDANCE,
     ...
     *
     * Tools pass through the Hermes allowlist first: computer + plan stay, bare unknowns drop,
     * granted MCP refs (`server/tool`) stay.
     */
    ...(tools.length > 0
      ? { tools: filterHermesTools(tools), maxSteps: TOOL_STEPS }
      : {}),
  };
}
