...
type Props = {
   ...
   */
  toolCallId?: string;
  /** `fill` = Lab full-bleed browser (no card / expand modal). */
  layout?: "card" | "fill";
};
...
export function ComputerView({
  ...
  finished,
  toolCallId,
  /** Lab / OneKey: fill the pane — no small card + modal. */
  layout = "card",
}: Props) {
  ...
  // Lab / OneKey: full-bleed pane — no small card, no expand modal.
  if (layout === "fill") {
    return (
      <div className="flex h-full min-h-0 w-full flex-1 flex-col bg-white text-neutral-900">
        <div className="relative min-h-0 flex-1 overflow-hidden bg-white">
          {showLiveScreen ? (
            <div className="absolute inset-0">
              ...
              />
              {problem ? (
                <div className="absolute inset-0 flex items-center justify-center bg-white/90 p-6 text-center text-sm text-neutral-500">
                  {problem}
                </div>
              ) : null}
            </div>
        ...
        {settled ? null : (
          <div className="flex shrink-0 items-center gap-3 border-t border-neutral-200 bg-neutral-50 px-3 py-2">
            <span className="min-w-0 flex-1 truncate text-[12px] text-neutral-500">
              {driving
                ? "Du steuerst — klicken und tippen auf der Seite."
                : problem
                  ? problem
                  : "Sandbox / Cloud-Browser"}
            </span>
            {driving ? (
              <button
                className="shrink-0 rounded-md bg-neutral-900 px-3 py-1.5 text-[12px] font-medium text-white"
              ...
              <button
                className="shrink-0 rounded-md border border-neutral-300 bg-white px-3 py-1.5 text-[12px] font-medium text-neutral-800"
                onClick={async () => {
