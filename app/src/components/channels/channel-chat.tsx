...
} from "@/lib/agents/model-provider-dispatch";
import { dispatchShellTurn } from "@/lib/agents/shell-dispatch";
...
export function channelHistoryNotice({
  ...
  historyAvailability: _historyAvailability,
  historyReadFailed: _historyReadFailed = false,
  ...
  // Connect: never show history-gap banners above the composer. Chat keeps working either way.
  void restoring;
  return null;
}
...
export function ChannelChat({
  ...
  const deliver = async (
    ...
    // Model One (External) or legacy shell bots — never hit Hermes AG-UI.
    if (shouldUseModelRouter(runtimeAgentId)) {
      const agentName =
        agentProfiles?.find((a) => a.id === runtimeAgentId)?.name ??
        runtimeAgentId;
      const fileHints = attachments
        .map((a) => {
          try {
            return uploadedAttachment(a).filename;
          } catch {
            return undefined;
          }
        })
        .filter((n): n is string => Boolean(n));
      const result = await dispatchModelTurn({
        agentId: runtimeAgentId,
        text: trimmed,
        agentName,
        fileHints: fileHints.length ? fileHints : undefined,
      });
      if (result.useHermes) {
        // Model Two selected globally — fall through to CopilotKit.
      } else {
        target.addMessage({
        ...
        report(result.summary, runtimeAgentId);
        awaitingReply.current = false;
        if (!result.ok) {
          turnFailure.current = result.summary;
          setRunError(result.summary);
          throw new Error(result.summary);
        }
        return;
      }
    } else if (agentUsesExternalShell(runtimeAgentId)) {
      const agentName =
        agentProfiles?.find((a) => a.id === runtimeAgentId)?.name ??
        runtimeAgentId;
      const result = await dispatchShellTurn({
        agentId: runtimeAgentId,
        text: trimmed,
        agentName,
      });
    ...
    >
      <ConversationProvider ask={askFromComponent}>
        <ConversationView
          ...
          onSubmit={async (draft) => {
            ...
            const mcpHint =
              draft.mcpServerIds.length > 0
                ? [
                    `Use these MCP servers for this turn: ${draft.mcpServerIds.join(", ")}. Prefer their tools when relevant.`,
                  ]
                : [];

            await say(
              draft.text,
              [...mcpHint, ...skillInstructions],
              draft.attachments,
            );
          }}
          /**
          ...
          /*
           * Hermes Phase A: a new message interrupts the in-flight turn instead of parking behind it.
           * Stop reaches the abort controller; the new draft starts immediately as its own turn.
           */
          interruptWhileBusy
          restoring={restoring}
          /*
