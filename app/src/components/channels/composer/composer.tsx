...
export type ComposerProps = {
  ...
  compact?: boolean;
  /**
   * Home floating bar: transparent chrome (shell draws the dark pill) and a
   * two-row layout so text does not collide with mic/send.
   */
  floating?: boolean;
  ...
  /**
   * When set with `onQueue`, a busy send stops the current turn instead of parking the message.
   * Changes the send button label so the person knows the running answer will be cut short.
   */
  interruptWhileBusy?: boolean;
  ...
  /**
   * Local Whisper mic + Meet-style waveform call button in the compact composer.
   * `onVoiceCall` opens the voice-call overlay; the mic fills the draft with a transcript.
   */
  enableVoice?: boolean;
  onVoiceCall?: () => void;
  /** When set, shows a per-bot model picker (ChatGPT / Claude / Cursor / …). */
  agentId?: string;
};
    ...
    return (
      ...
      <div className="flex flex-col" ref={containerRef} {...dropZone}>
        ...
        <form
          ...
          className={cn(
            floatingChrome
              ? "flex min-h-14 w-full flex-col rounded-2xl border border-border bg-card px-3 py-3 focus-within:border-ring focus-within:ring-3 focus-within:ring-ring/50"
          ...
          data-floating-composer={floatingChrome ? "" : undefined}
          ...
          {floatingChrome ? (
            <>
              <PromptArea
                aria-label="Message"
                className={cn(
                  "min-w-0 w-full border-0 bg-transparent p-0 text-sm shadow-none",
                  editorClassName,
                )}
                disabled={disabled}
                ...
                placeholder={voiceListening ? "Hört zu..." : "Ask anything"}
                ref={promptAreaRef}
                triggers={triggers}
                value={value}
              />
              <div className="flex items-center gap-1.5 pt-0.5">
                ...
                <div className="ml-auto flex items-center gap-1">
                  {enableVoice ? (
                    <ComposerMicButton
                      disabled={disabled}
                      onRecordingChange={setVoiceListening}
                      onTranscript={(spoken) => {
                        setValue((current) => {
                          if (isSegmentsEmpty(current)) {
                            return [text(spoken)];
                          }
                          return [...current, text(" "), text(spoken)];
                        });
                      }}
                    />
                  ) : null}
                  {enableVoice && onVoiceCall ? (
                    <VoiceCallButton onClick={onVoiceCall} />
                  ) : null}
                  {canStop ? (
                    <Button
                    ...
                    <Button
                      ...
                      className="size-8 rounded-full p-0 disabled:opacity-40"
                      disabled={!canSend}
                      size="icon"
                      type="submit"
                    >
