...
import { AgentComputerPeek } from "@/components/agents/agent-computer-peek";
...
export function ConversationView({
  ...
  /**
   * Hermes interrupt: a new message stops the in-flight turn and starts immediately.
   * Takes precedence over `queueWhileBusy` when both are set.
   */
  ...
  onStop?: () => void;
  /** Meet-style voice call target (company CTO or the channel coworker). */
  ...
  const submit = useCallback(
    (draft: ComposerDraft, whileBusy: boolean) => {
      // CLI runs in the background only (Model One) — never open a terminal UI.
      const next = apply({
        busy: whileBusy,
        draft,
        id: newId(),
    ...
    <div className="flex flex-col flex-1 min-h-0">
      ...
      <div className="max-w-2xl mx-auto w-full px-0 pb-4 shrink-0">
        ...
        {voiceAgentId ?? agents[0]?.id ? (
          <div className="px-1">
            <AgentComputerPeek agentId={voiceAgentId ?? agents[0]!.id} />
          </div>
        ) : null}
        <Composer
          ...
          interruptWhileBusy={interruptWhileBusy}
          onStop={onStop}
