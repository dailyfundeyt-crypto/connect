...
export function useStartChannel() {
  ...
  /**
   * Create the channel and stash the first message without navigating —
   * for parallel Unternehmen assignments (open Messages to continue each).
   */
  const startQuiet = async (agentId: string, text: string) => {
    await startWithChosen({
      agentId,
      text,
      record: routeMessage,
      start: async (id, body) => {
        const channel = await createChannel.mutateAsync([id]);
        queryClient.setQueryData(channelKeys.detail(channel.id), channel);
        stashFirstMessage(channel.id, body);
      },
    });
  };
