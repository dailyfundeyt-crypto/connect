...
import { ShellAskCard } from "./hermes-tools";
...
export function ComputerTools() {
  ...
  /**
   * Shell commands are HITL by default (Ask), matching MCP permissions.
   * Allow runs immediately; Deny refuses without executing; Ask shows Allow / Deny.
   */
  const ShellCommandCard = useMemo(
    () =>
      function ShellCommandRender(props: {
        args: Partial<{ command: string }>;
        ...
        const runCommand = async () => {
          if (!respond || !command) return;
          const outcome = await callComputer(
            computerId,
            "/exec",
        ...
        useEffect(() => {
          if (status !== "executing" || !respond || started.current) return;
          if (!command) return;
          const permission = readShellPermission();
          ...
          if (permission === "deny") {
            void respond({
              ...
              refused: true,
              reason:
                "Shell commands are denied for this session. Change Computer → Shell permission to Ask or Allow.",
            });
            return;
          }
          void runCommand();
        }, [status, respond, command, computerId]);
        ...
        if (status === "inProgress" || !respond) {
          return (
            <ToolLine
              running
              label="Preparing a command"
              detail={command || undefined}
            />
          );
        }

        if (readShellPermission() !== "ask") {
          return (
            <ToolLine running label="Running a command" detail={command} />
          );
        }
          ...
          <ShellAskCard
            ...
            onDeny={() => {
              ...
              void respond({
                ok: false,
                refused: true,
                reason: "The person declined this command.",
              });
            }}
          />
        );
  ...
  useHumanInTheLoop({
    ...
    description:
      ...
      "When Ask is on, the person must Allow or Deny before the command runs — wait for that.",
    parameters: z.object({
      command: z
        .string()
        .describe("The command to run, such as: sudo apt-get install -y jq"),
    }),
    render: ShellCommandCard,
  });
