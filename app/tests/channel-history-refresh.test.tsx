...
// History-gap banners are suppressed in Connect; tests assert transcript behaviour only.
...
test.each([
  ...
  async ({ snapshot }) => {
    ...
    await waitFor(() => expect(historyReads.length).toBeGreaterThan(0));
...
test("a stalled mount history read releases the send gate without a history banner", async () => {
  ...
  await waitFor(() => expect(historyReads.length).toBeGreaterThan(0), { timeout: 4000 });
...
test("mixed history, exhausted failure, and recovery keep messages without history banners", async () => {
  const view = await mounted();
  await act(async () => currentAgent().addMessage(local));
  ...
  await announce(2);
  // Three refresh attempts: immediate, +750ms, +1500ms.
  await new Promise((resolve) => setTimeout(resolve, 2300));
...
test.each([false, true])(
  ...
  async (prefixPresent) => {
    ...
    if (prefixPresent) await view.findByText(initial.content);
    else await waitFor(() => expect(historyReads.length).toBeGreaterThan(0));
    const user = userEvent.setup({ document: view.container.ownerDocument });
